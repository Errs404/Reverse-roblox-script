...
local ar = aq.px
local r = aq.scale
local lK = { frequency = 6 }
local function lL(C)
	local e0 = C.friend
	local c2 = C.index
	local aW = ei("home").friendActivity.friendButton
	local Z = aK(false)
	local l4 = Z[1]
	local b1 = Z[2]
	local lM = "https://www.roblox.com/headshot-thumbnail/image?userId="
		.. tostring(e0.VisitorId)
		.. "&width=48&height=48&format=png"
	local bf = { size = aO(l4 and ar(96, 48) or ar(48, 48), lK) }
	local G = {
		b.createElement(
			"ImageLabel",
			{
				Image = "rbxassetid://8992244272",
				ImageColor3 = aO(l4 and aW.accent or aW.dropshadow, lK),
				ImageTransparency = aO(l4 and aW.glowTransparency or aW.dropshadowTransparency, lK),
				Size = aO(l4 and ar(88 + 36, 74) or ar(76, 74), lK),
				Position = ar(-14, -10),
				ScaleType = "Slice",
		...
		b.createElement(
			bm,
			{ radius = 24, color = aO(l4 and aW.accent or aW.background, lK), transparency = aW.backgroundTransparency }
		),
	}
	local H = #G
	local I = aW.outlined
		and b.createFragment({
			border = b.createElement(
				bb,
				{ radius = 23, color = l4 and aW.foregroundAccent and aW.foregroundAccent or aW.foreground, transparency = 0.7 }
			),
		})
	if I then
	...
	G[H + 1] = b.createElement(
		"ImageLabel",
		{ Image = lM, ScaleType = "Crop", Size = ar(48, 48), LayoutOrder = c2, BackgroundTransparency = 1 },
		...
		{ clipsDescendants = true },
		{
			b.createElement(
				"ImageLabel",
				{
					...
					ImageColor3 = l4 and aW.foregroundAccent and aW.foregroundAccent or aW.foreground,
					ImageTransparency = aW.foregroundTransparency,
					Size = ar(36, 36),
					Position = ar(48, 6),
					BackgroundTransparency = 1,
				}
	...
	G[H + 3] = b.createElement("TextButton", {
		Text = "",
		...
		[b.Event.Activated] = function()
			pcall(function()
				ib:TeleportToPlaceInstance(e0.PlaceId, e0.GameId, dV.LocalPlayer)
			end)
		end,
